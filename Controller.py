...
def change_contact():
    ...
    choice2 = int(
        input('Что изменяем (0-имя, 1-фамилия, 2-отчество, 3-телефон): '))
...
def find_contact():
    ...
    find_el = (input('Введите строку для поиска: '))
    # print('\n')
    find = False
    
    for elem in Model.phonebook:

        if find_el.lower() in elem.lower():
            if not find:print("Нашли контакты:\n")
            print(elem)
            find = True
    if not find:
